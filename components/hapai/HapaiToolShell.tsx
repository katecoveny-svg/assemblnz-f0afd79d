...
export function HapaiToolShell({
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[linear-gradient(180deg,#FFF7EC_0%,#F7F1E9_54%,#FFF7EC_100%)] px-5 py-10 text-[#23211F] md:px-10 md:py-14">
      <div className="mx-auto max-w-[1500px]">
        ...
        <section className="mt-8 grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(340px,0.58fr)] xl:items-stretch">
          <div className="relative overflow-hidden rounded-[8px] border border-[rgba(35,33,31,0.10)] bg-white/62 p-6 shadow-[0_28px_90px_rgba(35,33,31,0.08)] md:p-8">
            <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(135deg,rgba(58,56,50,0.08),transparent_38%),radial-gradient(circle_at_86%_8%,rgba(217,168,90,0.16),transparent_30%)]" aria-hidden />
              ...
              {highlights.length > 0 ? (
                <div className="mt-7 grid gap-3 sm:grid-cols-3">
                  {highlights.map((item) => (
                    <div
                      key={item.title}
                      className="rounded-[8px] border border-[rgba(35,33,31,0.10)] bg-[#FFF7EC]/72 p-4 shadow-[0_18px_54px_rgba(35,33,31,0.05)] transition duration-300 hover:-translate-y-1 hover:border-[rgba(58,56,50,0.26)] hover:shadow-[0_24px_70px_rgba(58,56,50,0.12)]"
                    >
                      <div className="text-[#3A3832]">{item.icon ?? <Sparkles className="h-5 w-5" aria-hidden />}</div>
                      <p className="mt-4 font-mono text-[10px] uppercase tracking-[0.16em] text-[#3A3832]">
                        {item.title}
                      </p>
          ...
          <aside className="relative overflow-hidden rounded-[8px] border border-[rgba(35,33,31,0.10)] bg-white/64 p-3 shadow-[0_34px_110px_rgba(35,33,31,0.14)]">
            <div className="relative min-h-[270px] overflow-hidden rounded-[6px] border border-[rgba(35,33,31,0.08)] bg-[radial-gradient(circle_at_70%_18%,rgba(217,168,90,0.24),transparent_30%),linear-gradient(145deg,#FFF7EC_0%,#F3E9DC_48%,#FBF3DF_100%)] p-5">
              ...
              <div className="relative flex items-start justify-between gap-4">
                <div>
                  <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-[#3A3832]">how it works</p>
              ...
              <div className="absolute bottom-5 left-5 right-5 grid gap-2">
                {["paste or upload", "get the draft", "check and use"].map((label, index) => (
                  ...
                  >
                    <span>{label}</span>
                    <span className="font-mono text-[10px] text-[#3A3832]">0{index + 1}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="relative mt-3 overflow-hidden rounded-[6px] bg-[#23211F] p-5 text-[#FFF7EC]">
              ...
              <div className="mt-6 rounded-[8px] border border-[#FFF7EC]/16 bg-[#FFF7EC]/10 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-sm leading-relaxed text-[#FFF7EC]/82">{posture}</p>
                  </div>
                </div>
              </div>
              <Link
                href="/hapai#workflow-request"
                className="mt-5 inline-flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.14em] text-[#FFF7EC]/82 transition hover:text-[#FFF7EC]"
              >
                Suggest a better workflow
                <ArrowRight className="h-3.5 w-3.5" aria-hidden />
              </Link>
              <p className="mt-5 flex flex-wrap gap-x-3 gap-y-1 text-[11px] leading-relaxed text-[#FFF7EC]/58">
                <Link href="/privacy" className="hover:text-[#FFF7EC]">
                  Privacy
                </Link>
                <span aria-hidden>·</span>
                <Link href="/legal/terms" className="hover:text-[#FFF7EC]">
                  Terms
                </Link>
                <span aria-hidden>·</span>
                <Link href="/legal/disclaimer" className="hover:text-[#FFF7EC]">
        ...
        <section className="mt-6 rounded-[8px] border border-[rgba(35,33,31,0.10)] bg-white/54 p-3 shadow-[0_30px_100px_rgba(35,33,31,0.08)] transition duration-300 hover:shadow-[0_36px_120px_rgba(58,56,50,0.12)] md:mt-8 md:p-4">
          <div className="rounded-[6px] border border-[rgba(35,33,31,0.08)] bg-[#FFF7EC]">
            {children}
          </div>
        </section>
      </div>
