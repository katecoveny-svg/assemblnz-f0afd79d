    ...
    <main className="min-h-screen bg-[color:var(--assembl-paper)] px-6 py-12 text-[#313c42] md:px-12 md:py-16">
      <div className="mx-auto max-w-[920px]">
        ...
        <section className="mt-8 rounded-[14px] border border-[rgba(35,33,31,0.08)] bg-white p-7">
          <label className="block">
            ...
            <span className="flex min-h-[220px] cursor-pointer flex-col items-center justify-center rounded-[12px] border border-dashed border-[rgba(35,33,31,0.22)] bg-[#f7f9f8] p-5 text-center">
              {imageBase64 ? (
                ...
                <>
                  <Camera className="h-10 w-10 text-[#313c42]" />
          ...
          <button type="button" onClick={generate} disabled={loading || !imageBase64} className="mt-6 rounded-full bg-[#313c42] px-6 py-3 text-sm font-medium text-white hover:bg-[#313c42] disabled:bg-[#C8C2BC]">{loading ? "Reading the photo..." : copy.button}</button>
          <div className="mt-3">{gate.counter}</div>
          {gate.modal}
          {loading && <p className="mt-4 rounded-[10px] border border-[#b8964f]/30 bg-[#f7f9f8] px-4 py-3 text-sm text-[#6B5A28]">Building a kai plan from the photo.</p>}
        ...
        {result && (
          <section className="mt-8 rounded-[14px] border border-[rgba(35,33,31,0.08)] bg-white p-7">
            ...
            <h2 className="mt-8 font-display text-2xl font-normal text-[#313c42]">Suggested meals</h2>
            <div className="mt-3 grid gap-3">
              {result.meals.map((meal) => (
                <article key={meal.name} className="rounded-[10px] border border-[rgba(35,33,31,0.08)] bg-[#f7f9f8] p-4">
            ...
            <h2 className="mt-8 font-display text-2xl font-normal text-[#313c42]">Shopping list</h2>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <button type="button" onClick={() => navigator.clipboard.writeText(textOutput)} className="inline-flex items-center gap-2 rounded-full bg-[#313c42] px-5 py-3 text-sm font-medium text-white hover:bg-[#313c42]"><Copy className="h-4 w-4" /> Copy list</button>
              <button type="button" onClick={downloadTxt} className="inline-flex items-center gap-2 rounded-full border border-[rgba(35,33,31,0.18)] px-5 py-3 text-sm text-[#5A5550] hover:text-[#313c42]"><Download className="h-4 w-4" /> Download .txt</button>
...
function ResultList({ title, items, empty, compact = false }: { title: string; items: string[]; empty: string; compact?: boolean }) {
  return (
    <div className={compact ? "rounded-[10px] border border-[rgba(35,33,31,0.08)] bg-[#f7f9f8] p-4" : "mt-6"}>
