...
export function InstallPwaButton({ label = "Save to home screen", compact = false }: InstallPwaButtonProps) {
  ...
  if (installed) {
    return (
      <span className="inline-flex min-h-10 items-center gap-2 rounded-full border border-white/60 bg-white/42 px-4 text-sm text-[#313c42] shadow-[inset_0_1px_0_rgba(255,255,255,0.74)] backdrop-blur-md">
  ...
  return (
    <div className={compact ? "space-y-2" : "space-y-3"}>
      <button
        type="button"
        onClick={install}
        className="inline-flex min-h-10 items-center gap-2 rounded-full border border-white/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.74),rgba(232,239,233,0.58))] px-4 text-sm font-medium text-[#313c42] shadow-[inset_0_1px_0_rgba(255,255,255,0.88),0_12px_30px_rgba(58,56,50,0.10)] transition hover:-translate-y-0.5 hover:border-[rgba(58,56,50,0.34)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#313c42] focus-visible:ring-offset-2"
      >
