...
export function FluxConnections({ leads }: { leads: Lead[] }) {
  ...
  return <section className="sp-paper sp-connections" id="flux-connections">
    ...
    <div className="sp-workflow-tabs" role="group" aria-label="Business action">{([['salesforce-lead', 'Salesforce lead'], ['outlook-draft', 'Outlook email draft']] as const).map(([value, title]) => <button key={value} aria-pressed={kind === value} onClick={() => { setKind(value); setReviewed(false); setQueued(false); setNotice(''); }}>{title}</button>)}</div>
