    ...
    <div className={[
      ...
      >
        <div className="flex items-baseline gap-3">
          <span className="font-mono text-[12px] uppercase tracking-[0.22em] text-[color:var(--text-secondary)]">activity + test</span>
          <span className="rounded-[2px] bg-[color:var(--assembl-gold-thread)]/20 px-1.5 py-0.5 font-mono text-[12px] uppercase tracking-[0.14em] text-[color:var(--text-primary)]">demonstration</span>
        ...
        <div className="grid flex-1 grid-cols-1 gap-4 overflow-y-auto p-4 md:grid-cols-[1.05fr_1fr_0.9fr]">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              <div className="font-mono text-[12px] uppercase tracking-[0.22em] text-[color:var(--text-secondary)]">customer message · simulated</div>
              <pre className="whitespace-pre-wrap rounded-[3px] border border-[color:var(--assembl-cloud)] bg-[color:var(--assembl-paper)] p-3 font-mono text-[12px] leading-[1.55] text-[color:var(--text-primary)]">{REFUND_MESSAGE}</pre>
            </div>
            {currentIdx >= stageIndex('drafting') && (
              <div className="flex flex-col gap-1.5">
                ...
                <pre className="whitespace-pre-wrap rounded-[3px] border border-[color:var(--assembl-cloud)] bg-[color:var(--assembl-paper)] p-3 font-mono text-[12px] leading-[1.6] text-[color:var(--text-primary)]">{REFUND_DRAFT}</pre>
              </div>
            )}
            {currentIdx >= stageIndex('source-found') && (
              <div className="flex flex-col gap-1.5">
                <div className="font-mono text-[12px] uppercase tracking-[0.22em] text-[color:var(--text-secondary)]">supporting knowledge</div>
                <div className="rounded-[3px] border border-[color:var(--assembl-cloud)] bg-[color:var(--assembl-paper)] p-3">
                  <div className="font-mono text-[12px] text-[color:var(--text-primary)]">{REFUND_SOURCE.title}</div>
                  <div className="font-mono text-[12px] text-[color:var(--text-secondary)]">{REFUND_SOURCE.citation}</div>
                  <p className="mt-2 text-[12px] leading-[1.55] text-[color:var(--text-primary)]">{REFUND_SOURCE.snippet}</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col gap-3">
            <div className="font-mono text-[12px] uppercase tracking-[0.22em] text-[color:var(--text-secondary)]">stages</div>
                ...
                return (
                  <li key={s.id} className="flex items-baseline gap-2">
                    ...
                    <div>
                      ...
                      {state === 'current' && (
                        <div className="font-mono text-[12px] text-[color:var(--text-secondary)]">{s.hint}</div>
                      )}
                    </div>
                  </li>
                );
            ...
            {showApprovalUI && (
              <div className="mt-2 flex flex-col gap-2 rounded-[3px] border border-[color:var(--assembl-gold-thread)] bg-[color:var(--assembl-gold-thread)]/10 p-3">
                <div className="font-mono text-[12px] uppercase tracking-[0.22em] text-[color:var(--text-primary)]">approval required</div>
                <p className="text-[12px] leading-[1.55] text-[color:var(--text-primary)]">
                  koro won&rsquo;t send the reply until you say yes. Read the draft on the left, then choose.
                </p>
                <div className="flex gap-2">
                  <button type="button" onClick={approve}
                    className="rounded-[2px] border border-[color:var(--assembl-pounamu)] bg-[color:var(--assembl-pounamu)] px-3 py-1.5 font-mono text-[12px] uppercase tracking-[0.16em] text-[color:var(--assembl-paper)]">
                    Approve &amp; send
                  </button>
                  <button type="button" onClick={reject}
                    className="rounded-[2px] border border-red-400 bg-[color:var(--assembl-paper)] px-3 py-1.5 font-mono text-[12px] uppercase tracking-[0.16em] text-red-600">
                    Reject
                  </button>
                </div>
              </div>
            ...
            {test.approved === false && (
              <div className="rounded-[3px] border border-red-300 bg-red-50 p-3 text-[12px] text-red-700">
          ...
          <div className="flex flex-col gap-3">
            <div className="font-mono text-[12px] uppercase tracking-[0.22em] text-[color:var(--text-secondary)]">activity trace</div>
            <div className="flex-1 overflow-y-auto rounded-[3px] border border-[color:var(--assembl-cloud)] bg-[color:var(--assembl-paper)]">
              {test.activity.length === 0 ? (
                <p className="p-3 font-mono text-[12px] text-[color:var(--text-secondary)]">No activity yet. Press Run demo.</p>
              ) : (
                <ul className="divide-y divide-[color:var(--assembl-cloud)]">
                  {test.activity.map((e, i) => (
                    <li key={i} className="flex items-baseline gap-3 px-3 py-2 font-mono text-[12px]">
            ...
            <div className="flex flex-col gap-2 rounded-[3px] border border-[color:var(--assembl-cloud)] bg-[color:var(--assembl-paper)] p-3">
              ...
              {warnings.length > 0 ? (
                <ul className="flex flex-col gap-1">
                  {warnings.map((w, i) => (
                    <li key={i} className="font-mono text-[12px] text-[color:var(--st-warning,#a86b3d)]">⚠ {w}</li>
                  ))}
                </ul>
              ) : (
                <p className="font-mono text-[12px] text-[color:var(--st-success,#4d7468)]">All components configured.</p>
              )}
              <button
                type="button"
                disabled={warnings.length > 0}
                className="self-start rounded-[2px] border border-[color:var(--assembl-pounamu)] bg-[color:var(--assembl-pounamu)] px-3 py-1.5 font-mono text-[12px] uppercase tracking-[0.16em] text-[color:var(--assembl-paper)] disabled:opacity-50"
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[3px] border border-[color:var(--assembl-cloud)] bg-[color:var(--assembl-paper)] p-2">
      <div className="text-[12px] uppercase tracking-[0.18em] text-[color:var(--text-secondary)]">{label}</div>
