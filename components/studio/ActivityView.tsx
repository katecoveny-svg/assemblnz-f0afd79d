  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto bg-[color:var(--assembl-paper)] p-6 md:p-10">
      <div className="mx-auto flex w-full max-w-[900px] flex-col gap-6">
        <header>
          <p className="font-mono text-[12px] uppercase tracking-[0.24em] text-[color:var(--text-secondary)]">activity trace</p>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="ml-auto grid grid-cols-3 gap-2 font-mono text-[12px]">
        ...
        {test.activity.length === 0 ? (
          <p className="rounded-[3px] border border-[color:var(--assembl-cloud)] bg-[color:var(--assembl-paper)] p-4 font-mono text-[12px] text-[color:var(--text-secondary)]">
            No activity yet. Press Run demo above.
          </p>
        ) : (
          <ol className="flex flex-col gap-1.5">
            {test.activity.map((e, i) => (
              <li key={i} className="rounded-[3px] border border-[color:var(--assembl-cloud)] bg-[color:var(--assembl-paper)] p-3 font-mono text-[12px]">
                <div className="flex items-baseline gap-3">
                  <span className="w-12 shrink-0 text-right text-[color:var(--text-secondary)]">{(e.at / 1000).toFixed(1)}s</span>
                  <span className="font-mono text-[12px] uppercase tracking-[0.16em] text-[color:var(--text-secondary)]">{e.stage}</span>
                ...
                {e.active.length > 0 && (
                  <div className="mt-1 pl-16 text-[12px] text-[color:var(--text-secondary)]">
