...
> = {
  "api-key": {
    ...
    useCasePlaceholder:
      "e.g. Commerce Commission decisions for our compliance dashboard",
  },
  "talk-to-kate": {
    ...
    useCasePlaceholder:
      "e.g. a sanctions-screening step inside our onboarding flow",
  },
};
    ...
    try {
      const res = await fetch("/api/data-waitlist", {
        ...
        body: JSON.stringify({
          ...
          organisation:
            String(data.get("organisation") ?? "").trim() || undefined,
      ...
      setError(
        "Network hiccup. Please try again, or email assembl@assembl.co.nz.",
      );
      setStatus("error");
    }
  }
    ...
    >
      ...
      {status === "done" ? (
        <div className="mt-6 flex items-start gap-3 rounded-[8px] border border-[rgba(58,56,50,0.3)] bg-[color:var(--assembl-pounamu-paper)] p-5">
          <Check
            className="mt-0.5 h-5 w-5 shrink-0 text-[color:var(--assembl-pounamu)]"
            aria-hidden
          />
          <div>
        ...
        <form onSubmit={onSubmit} className="mt-6 space-y-4" noValidate>
          ...
          <div>
            <label
              htmlFor={useCaseId}
              className="block font-mono text-[11px] uppercase tracking-[0.14em] text-[color:var(--text-secondary)]"
            >
              {copy.useCaseLabel}
            </label>
            <textarea
              ...
              className="mt-2 w-full rounded-[8px] border border-[rgba(35,33,31,0.16)] bg-[#FFF7EC] px-4 py-3 text-sm text-[color:var(--text-primary)] placeholder:text-[color:var(--text-secondary)]/70 focus-visible:ring-2 focus-visible:ring-[color:var(--assembl-pounamu)]/30 focus-visible:ring-offset-1 focus-visible:border-[color:var(--assembl-pounamu)] focus:outline-none"
            />
            <div className="mt-1.5 flex justify-end">
              ...
              <span className="sr-only" aria-live="polite">
                {useCase.length >= 900
                  ? `${1000 - useCase.length} characters remaining`
                  : ""}
              </span>
            </div>
          </div>
          ...
          <p className="text-xs leading-relaxed text-[color:var(--text-secondary)]">
            Your details reach a named human in Aotearoa. No spam, no resale —
            just your key and the occasional product note.
          </p>
        </form>
      )}
    </div>
  ...
  return (
    <div>
      ...
      >
        ...
        {required && (
          <span
            className="text-[color:var(--assembl-pounamu)] ml-1"
            aria-hidden="true"
          >
            *
          </span>
