...
const doDoor = PRODUCT_DESTINATIONS.do.workspace;
const studioWorkspace = PRODUCT_DESTINATIONS.studio.workspace;

const productHrefs = {
  pursuit: { href: pursuitHub, external: true },
  do: { href: doDoor, external: false },
  studio: { href: studioWorkspace, external: true },
} as const;
...
export function AssemblTheWorkHome({ preview = false }: { preview?: boolean }) {
  return <div className="atw atw-spatial" data-preview={preview}>
    ...
    <section className="atw-product-strip" aria-label="Three products, one system">
      <a href={pursuitHub} target="_blank" rel="noopener noreferrer"><strong>Pursuit</strong><span>— <em>find it.</em></span></a>
      <Link href={doDoor}><strong>DO</strong><span>— <em>do it.</em></span></Link>
      <a href={studioWorkspace} target="_blank" rel="noopener noreferrer"><strong>Studio</strong><span>— <em>show it.</em></span></a>
      <p>{HERO.loopLine}</p>
    </section>
    ...
    <section className="atw-product-list atw-section" aria-label="Explore the products">
      {PRODUCTS.items.map((product, index) => {
        const door = productHrefs[product.id as keyof typeof productHrefs];
        return (
          <article className="atw-product-row" id={product.id} key={product.id}>
            <span className="atw-product-number">0{index + 1}</span>
            <div className="atw-product-name">
              <p>{product.verb.split('·')[1]?.trim()}</p>
              <h2>{product.name}</h2>
            </div>
            <div className="atw-product-description">
              <p>{product.body}</p>
              {door.external ? (
                <a className="atw-text-link" href={door.href} target="_blank" rel="noopener noreferrer">
                  {product.explore}
                  <ArrowUpRight size={20} />
                </a>
              ) : (
                <Link className="atw-text-link" href={door.href}>
                  {product.explore}
                  <ArrowUpRight size={20} />
                </Link>
              )}
    ...
    <section className="atw-choose atw-section" id="choose"><p className="atw-kicker">START WHERE THE WORK IS.</p><h2>one product.<br />or the <em>whole system.</em></h2><p>Start with Pursuit or Studio. Connect DO when you want the full loop from signal to execution to proof.</p><div><a className="atw-pill atw-pill-dark" href={pursuitHub} target="_blank" rel="noopener noreferrer">Open Pursuit <ArrowUpRight size={19} /></a><Link className="atw-text-link" href="/contact?product=system">Plan the complete system <ArrowUpRight size={19} /></Link></div><small>Connections, permissions and delivery are agreed before consequential actions are enabled.</small></section>
    <footer className="atw-footer"><Link className="atw-wordmark" href="/">assembl</Link><p>Find it. DO it. Show it.<br />Built in New Zealand.</p><nav aria-label="Footer"><a href={pursuitHub} target="_blank" rel="noopener noreferrer">Pursuit</a><Link href={doDoor}>DO</Link><a href={studioWorkspace} target="_blank" rel="noopener noreferrer">Studio</a><Link href="/contact">Contact</Link><Link href="/legal/privacy">Privacy</Link></nav><span>One shared context · portable agents · proof that compounds.</span></footer>
  </div>;
}
