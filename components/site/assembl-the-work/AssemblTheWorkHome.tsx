...
const LOOP = [
  { name: 'Pursuit', verb: 'find it.', detail: 'live signals → qualified work', href: '/pursuit' },
  { name: 'DO', verb: 'DO it.', detail: 'portable agents → execution', href: '/do' },
  { name: 'Studio', verb: 'show it.', detail: 'proof → pitch → experience', href: '/creative-studio' },
] as const;
...
export function AssemblTheWorkHome({ preview = false }: { preview?: boolean }) {
  return <div className="atw" data-preview={preview}>
    ...
    <section className="atw-hero" aria-labelledby="atw-hero-title">
      <div className="atw-hero-field" aria-hidden="true" />
      <header className="atw-nav">
        <Link className="atw-wordmark" href="/" aria-label="assembl home">assembl</Link>
      ...
      <div className="atw-hero-copy">
        <p className="atw-hero-overline">ASSEMBL THE WORK</p>
        <h1 id="atw-hero-title">find it.<br />DO it.<br />show it.</h1>
        <p className="atw-hero-sub">{HERO.subhead}</p>
        <p className="atw-hero-body">One shared system for signals, specialist agents, execution and proof.</p>
        <div className="atw-hero-actions">
          <a className="atw-pill atw-pill-light" href="#do-input" onClick={() => document.getElementById('atw-do-intent')?.focus()}>Give DO a job <ArrowRight size={21} /></a>
          <a className="atw-hero-link" href="#products">See the system <ArrowDown size={17} /></a>
        </div>
      </div>

      <aside className="atw-hero-system" aria-label="Assembl product system">
        {LOOP.map((item, index) => <Link key={item.name} href={item.href} className="atw-hero-system-row">
          <span>0{index + 1}</span><div><strong>{item.name}</strong><em>{item.verb}</em><small>{item.detail}</small></div><ArrowUpRight size={16} />
        </Link>)}
        <p><strong>Factory underneath.</strong> Shared context · connectors · models · permissions · proof.</p>
      </aside>
    ...
    <section className="atw-system atw-section" aria-labelledby="atw-system-title">
      <div className="atw-system-copy"><p className="atw-kicker">ONE CONTEXT. MANY MODELS. VISIBLE PROOF.</p><h2 id="atw-system-title">the model can change.<br />the work remembers.</h2><p>DO is the portable execution layer. It can use different models for coding, research, voice, vision or creative work while keeping the same context, tools, permissions and evidence.</p><Link className="atw-text-link" href="/do">Meet DO <ArrowUpRight size={18} /></Link></div>
      <ol className="atw-system-loop">
        <li><span>01</span><strong>FIND</strong><p>Pursuit brings evidence-backed opportunities into view.</p></li>
        <li><span>02</span><strong>DO</strong><p>Your DOs research, prepare, build and coordinate around the work.</p></li>
        <li><span>03</span><strong>SHOW</strong><p>Studio turns the result into something people can see, try and understand.</p></li>
        <li><span>04</span><strong>LEARN</strong><p>Proof and decisions feed the next job instead of disappearing into a chat.</p></li>
      </ol>
    </section>

    <section className="atw-how atw-section" id="how-it-works"><div><p className="atw-kicker">THE WORK MOVES. YOU STAY IN CONTROL.</p><h2>context in.<br />work out.<br /><em>proof attached.</em></h2></div><ol><li><span>01</span><div><h3>Bring what matters.</h3><p>Your goal, business context and the sources you choose to connect.</p></div></li><li><span>02</span><div><h3>Let the right DO take it.</h3><p>Assembl routes the job to the right specialist agents, tools and model for the work.</p></div></li><li><span>03</span><div><h3>Review what happens next.</h3><p>Approvals, evidence and receipts stay visible. Sending, publishing and spending remain permissioned actions.</p></div></li></ol></section>
