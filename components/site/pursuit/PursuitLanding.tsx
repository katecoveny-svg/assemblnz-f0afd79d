...
export function PursuitLanding() {
  ...
  return <div className={`atw ${styles.page}`}>
    <a href="#try-pursuit" className="atw-skip">Skip to public research</a>
    <section className={styles.hero} aria-labelledby="pursuit-title"><div className={styles.ambient} aria-hidden="true" /><header className={styles.header}><Link href="/" className={styles.wordmark}>assembl</Link><nav aria-label="Primary"><Link href="/pursuit" aria-current="page">Pursuit</Link><Link href="/do">DO</Link><Link href="/creative-studio" >Studio</Link></nav></header>
      <div className={styles.heroGrid}><div className={styles.heroCopy}><p className={styles.eyebrow}>Pursuit / find it.</p><h1 id="pursuit-title">See what’s changing.<br /><span>Find your next move.</span></h1><p className={styles.lead}>Assembl connects public APIs, official records and company news. Pursuit helps you research what those changes could mean for a business, then prepare a specific proposal.</p><a className={styles.primary} href="#try-pursuit">Research an opportunity <ArrowRight size={19} /></a><a className={styles.quietLink} href={hub} target="_blank" rel="noopener noreferrer">Open private client hubs <ArrowUpRight size={16} /></a><p className={styles.honesty}>A free public research trial. Review the sources and draft before you contact anyone.</p></div>
      <div id="pursuit-canvas" className={styles.example} aria-label="Assembl sources and an example research path"><PursuitCanvasHero /></div></div>
      <div className={styles.heroFoot}><span>Public signals. Business context. Work you can propose.</span><Link href="/tools/agents">For agents and developers</Link></div>
    </section>
    <LivePursuitCanvas />
    <WebsiteOutreach />
