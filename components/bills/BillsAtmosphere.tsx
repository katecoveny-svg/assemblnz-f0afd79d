...
/**
 * The cosmic backdrop for Assembl Bills — drifting nebula orbs (teal / coral /
 * gold) plus a faint particle field with the brand's constellation lines. Sits
 * behind everything at z-0; pointer-events off. Honours prefers-reduced-motion
 * (static render, no rAF loop).
 */
export function BillsAtmosphere() {
  ...
  useEffect(() => {
    ...
    const TINTS = ['90,173,160', '242,130,94', '233,196,106', '255,255,255', '127,178,200'];
    ...
    const build = () => {
      const bg: P[] = Array.from({ length: Math.min(160, Math.round((W * H) / 14000)) }, () => ({
        x: rand(0, W), y: rand(0, H), r: rand(0.3, 1.5), op: rand(0.1, 0.7), spd: rand(0.3, 1.6), ph: rand(0, Math.PI * 2), col: '255,255,255', node: false,
      }));
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden" aria-hidden>
      {/* Nebula orbs */}
      <div className="orb-b" style={{ width: 520, height: 420, top: '-8%', left: '-6%', background: 'radial-gradient(circle, rgba(90,173,160,0.16), transparent 70%)', ['--d' as string]: '17s' }} />
      <div className="orb-b" style={{ width: 460, height: 460, top: '20%', right: '-10%', background: 'radial-gradient(circle, rgba(242,130,94,0.13), transparent 70%)', ['--d' as string]: '21s', ['--del' as string]: '-4s' }} />
      <div className="orb-b" style={{ width: 380, height: 300, bottom: '-6%', left: '25%', background: 'radial-gradient(circle, rgba(233,196,106,0.09), transparent 70%)', ['--d' as string]: '25s', ['--del' as string]: '-9s' }} />
      {/* Particle field */}
      <canvas ref={canvasRef} className="absolute inset-0 h-full w-full opacity-70" />
      {/* Fine grid + vignette */}
      <div className="absolute inset-0" style={{ background: 'radial-gradient(120% 80% at 50% -10%, transparent 40%, rgba(4,6,12,0.55) 100%)' }} />
      <style>{`
