...
An attempted officially supported `pnpm build --webpack` verification path (https://nextjs.org/docs/app/api-reference/turbopack) passed 26 CI tests but failed on an existing `@splinetool/react-spline` package export in `components/brand/SplineScene.tsx`. The workaround was removed and the original production build command restored. Neither build issue is claimed fixed. Browser CI did not run, so no desktop/mobile screenshots or visual pass are claimed. The Vercel preview requires sign-in in the available cloud browser; no authentication protection was removed. The PR stays draft pending visual and live-provider validation.
