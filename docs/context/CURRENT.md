...
**Task DO Maker (16 Sep 2026):** white-label task-specific DO minting at `/studio/do-maker` with two modes — **Pursuit/Studio** (Mode A, opportunity handoff from `/pursuit`) and **partner-facing** (Mode B, `/do/maker/partner/[slug]` alias, offline `bp` + `warehouse` skins). **bp Road-Ready** defaults to the interactive **sponsored-agent** walkthrough on the same maker path — vertical LOCK = **fuel / convenience retail loyalty** (wait-at-pump / app → points / nearby / genuine offer → DO Permit → DEMO action → receipt). **Not** electricity switch. ASA “Sponsored” grammar reused from Assembling (no Dash fork). PREVIEW only — no live bp partnership. Not a parallel `/demos/bp-*` route. Exports portable AgentSpec + preview URL; browser/session drafts only unless durable Office save is explicitly available.
