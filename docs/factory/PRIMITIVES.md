...
| Studio Task DO Maker | active foundation | `lib/studio/task-do-maker.ts`, `lib/studio/bp-sponsored-journey.ts`, `/studio/do-maker`, `/do/maker/partner/*` | Pursuit overview handoff + partner-facing skins + bp sponsored-agent fuel-loyalty walkthrough + DO Office session intake | `lib/studio/task-do-maker.test.ts`, `lib/studio/bp-sponsored-journey.test.ts` | shared core for Mode A (Pursuit pitch) and Mode B (partner chrome, sponsored-agent / rewarded-wait); offline `bp`/`warehouse` skins; drafts-only AgentSpec export; Assembl Permit + receipt differentiator vs ChatGPT Ads |
