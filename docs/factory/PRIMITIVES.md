...
| Studio Task DO Maker | active foundation | `lib/studio/task-do-maker.ts`, `/studio/do-maker` | Pursuit overview handoff + DO Office session intake | `lib/studio/task-do-maker.test.ts` | white-label task DO minting; drafts-only AgentSpec export; browser/session persistence honest about non-durable saves |
