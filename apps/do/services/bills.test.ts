...
describe('bill comparison boundaries', () => {
  ...
  it('rejects a model answer with no actual search evidence', () => { expect(()=>readGroundedBillResult({candidates:[{content:{parts:[{text:'A cheap plan exists'}]}}]})).toThrow(); });
  it('requires both queries and safe source URLs', () => {
    const response=(uri:string,queries:string[])=>({candidates:[{content:{parts:[{text:'Review this advertised plan.'}]},groundingMetadata:{groundingChunks:[{web:{uri,title:'Provider'}}],webSearchQueries:queries}}]});
