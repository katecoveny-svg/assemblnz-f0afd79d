...
export function doWidgetScript(origin: string): string {
  ...
  /* Spatial C: plum stage orb + rose glow only — Instrument Sans stack. */
  style.textContent = ":host{all:initial}button{font:14px 'Instrument Sans',ui-sans-serif,system-ui,sans-serif;cursor:pointer}.launch{position:fixed;right:18px;bottom:12px;z-index:2147483000;width:100px;height:84px;border:0;padding:0;background:transparent;filter:drop-shadow(0 0 14px #d6a5bd80);transition:transform .2s}.launch:hover{transform:translateY(-3px)}.launch .spark{display:grid;place-items:center;width:66px;height:66px;margin:auto;border-radius:22px;color:#f3d4de;background:radial-gradient(ellipse at 25% 15%,#c995a8,#916a70 42%,#391333 72%,#240b21);box-shadow:inset 1px 1px 2px #f8e4ea8c,inset -2px -2px 5px #180a1e,0 0 22px #d6a5bd80}.launch .spark svg{width:42px;height:42px;filter:drop-shadow(0 0 4px #e8b6c4)}.launch{touch-action:none;cursor:grab}.panel{position:fixed;right:20px;bottom:86px;width:min(470px,calc(100vw - 24px));height:min(740px,calc(100dvh - 110px));z-index:2147483001;background:#fffdfb;border:1px solid #916a7070;border-radius:13px;box-shadow:0 12px 60px #240b2150;overflow:hidden}.close{height:36px;width:36px;position:absolute;right:8px;top:8px;border:1px solid #240b2120;background:#fffdfb;border-radius:50%;z-index:2;color:#240b21}.panel iframe{width:100%;height:100%;border:0}button:focus-visible{outline:2px solid #916a70;outline-offset:4px}[hidden]{display:none!important}";
  const launch = document.createElement('button');
  launch.className = 'launch'; launch.type = 'button'; const badge = document.createElement('span'); badge.className='spark'; badge.setAttribute('aria-hidden','true'); badge.innerHTML = '<svg viewBox="0 0 64 64"><path d="M16 12H29C44 12 52 20 52 32S44 52 29 52H16Z" fill="none" stroke="currentColor" stroke-width="7" stroke-linejoin="round"/><circle cx="30" cy="32" r="6" fill="currentColor"/></svg>'; launch.append(badge);
