...
(() => {
  ...
  function restore(element,key){try{let saved=JSON.parse(localStorage.getItem(positionKey)||'null')?.[key];if(!saved&&key==='launch'){const old=JSON.parse(localStorage.getItem("assembl:do:companion-position:v1")||'null');if(old)saved={x:old.left,y:old.top};}if(saved&&Number.isFinite(saved.x)&&Number.isFinite(saved.y))position(element,saved.x,saved.y);}catch{}}
