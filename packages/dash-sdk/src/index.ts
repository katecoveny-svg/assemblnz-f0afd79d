/**
 * @assembl/dash-sdk — the client for Dash by assembl, the NZ in-product ad network.
 *
...
export interface DashConfig {
  ...
  publisherId: string;
  /** Ad-server base URL. Defaults to the live Dash by assembl endpoint. */
  endpoint?: string;
}

export interface DashShowOptions {
...
const DEFAULT_ENDPOINT = 'https://www.assembl.co.nz/api/dash';

let config: Required<DashConfig> | null = null;

function endpointBase(): string {
  if (!config) throw new Error('[dash] call dash.init({ publisherId }) before show().');
  return config.endpoint.replace(/\/$/, '');
}

export const dash = {
  /** Configure the SDK once, at startup. */
  init(cfg: DashConfig): void {
    if (!cfg?.publisherId) throw new Error('[dash] publisherId is required.');
   ...
   */
  async show(opts: DashShowOptions): Promise<DashAd | null> {
    if (!opts?.surface) throw new Error('[dash] show({ surface }) is required.');
    ...
    try {
      ...
      const ad = (await res.json()) as Partial<DashAd> | null;
