...
/**
 * Agent SMS — Generic agent inbound handler.
 * Now forwards all inbound messages to the unified tnz-inbound gateway
 * which handles Iho routing to the correct agent based on message content.
 */
Deno.serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ ok: true, forwarded: true, source: "agent-sms", ...result }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (err) {
    console.error("[agent-sms] Error:", err);
