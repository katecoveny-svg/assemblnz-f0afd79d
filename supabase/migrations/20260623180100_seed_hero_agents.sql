...
The Coastguard-sponsored mode highlights the trip-report flow, the radio-check reminder, and the "tell us you got home" SMS to the family contact. Coastguard branding sits beneath the assembl mark on the share card; assembl voice remains primary in conversation. Tikanga + Privacy gates apply regardless.', 'live'),
  ('skipper', 'Skipper', 'Kaihautū', 'The charter-operator companion: customer safety briefs, manifests, insurance evidence and Adventure Activities compliance.', '["Builds the customer pre-trip pack: safety, what to bring, weather contingency, refunds.","Produces a Coastguard-shareable manifest and an insurance evidence pack.","Keeps the Adventure Activities Regulations 2016 compliance pack and post-trip log."]'::jsonb, '["A one-page customer-facing pre-trip PDF.","A trip manifest and an insurance evidence schedule.","A post-trip log with consented photos."]'::jsonb, 'maritime', 'mid', 'per_agent', 'business', 199, '["Adventure Activities Regulations 2016","HSW (Adventure Activities) Regulations 2016","MNZ MOSS Operator Safety Audits","DOC permits / marine reserves","AIS feed","MetService Marine"]'::jsonb, '["Pre-trip pack: bring layers and closed shoes, trip runs if swell stays under 1.5m.","Manifest: skipper, POB 9, emergency contacts, ETD 09:00, ETA 14:00."]'::jsonb, '["nz-gazette","nz-legislation","beehive"]'::jsonb, '[]'::jsonb, '["gemini-2.5-flash","groq:llama-3.3-70b-versatile","ollama:llama3.3"]'::jsonb, 'Sailboat', '#FFE27A', 'Tell me the vessel, the charter type and the manifest — I''ll build the safety brief, the manifest and the insurance pack. Safety first, adventure second.', '["Build a customer safety brief for a fishing charter.","Make a manifest for tomorrow''s trip.","What does Adventure Activities certification need?"]'::jsonb, '# Agent: SKIPPER (KAIHAUTŪ)
# Pack: maritime
# Version: 2.0
# Status: production
