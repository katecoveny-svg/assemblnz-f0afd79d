-- Seed — live agent roster (24 agents).
--
...
DELETE FROM public.agent_installs WHERE agent_slug NOT IN ('9am-brief', 'fridge-to-list', 'panui-parser', 'whanau-help', 'school-notice', 'care-captain', 'invoice-tidy', 'hui-notes', 'roster-sorter', 'inbox-triage', 'travel-logs', 'tax-tidy', 'meeting-records', 'power-watch', 'customs-entry', 'food-temp-logs', 'stock-count', 'compliance-check', 'maritime-brief', 'tide-weather', 'catch-log', 'care-scribe', 'voice-cs', 'creative-studio');
DELETE FROM public.agent_chat_sessions WHERE agent_slug NOT IN ('9am-brief', 'fridge-to-list', 'panui-parser', 'whanau-help', 'school-notice', 'care-captain', 'invoice-tidy', 'hui-notes', 'roster-sorter', 'inbox-triage', 'travel-logs', 'tax-tidy', 'meeting-records', 'power-watch', 'customs-entry', 'food-temp-logs', 'stock-count', 'compliance-check', 'maritime-brief', 'tide-weather', 'catch-log', 'care-scribe', 'voice-cs', 'creative-studio');
DELETE FROM public.agents WHERE slug NOT IN ('9am-brief', 'fridge-to-list', 'panui-parser', 'whanau-help', 'school-notice', 'care-captain', 'invoice-tidy', 'hui-notes', 'roster-sorter', 'inbox-triage', 'travel-logs', 'tax-tidy', 'meeting-records', 'power-watch', 'customs-entry', 'food-temp-logs', 'stock-count', 'compliance-check', 'maritime-brief', 'tide-weather', 'catch-log', 'care-scribe', 'voice-cs', 'creative-studio');
...
Warm and professional, a calm voice when the office is closed.', 'live'),
  ('creative-studio', 'Creative Studio', 'Auaha', 'Brief to finished asset, in one chat.', '["Takes a brief through copy, image, video, podcast and one-shot apps.","Writes campaign copy, captions, taglines and scripts in NZ English.","Keeps the work on brand and flags what needs a licence or consent."]'::jsonb, '["Drafted copy and produced assets, marked by pipeline stage.","A list of approvals and licences needed before anything ships.","A hand-off to scheduling once an asset is signed off."]'::jsonb, 'business', 'premium', 'per_agent', 'business', 199, '["Copyright Act 1994","Advertising Standards Authority Code","Privacy Act 2020"]'::jsonb, '["Three tagline options, plus a 1:1 hero concept brief for sign-off.","Storyboard + script for an 8-second promo; render is a follow-up."]'::jsonb, '["nz-gazette","nz-legislation","beehive"]'::jsonb, '[]'::jsonb, '["gemini-2.5-flash","groq:llama-3.3-70b-versatile","ollama:llama3.3"]'::jsonb, 'palette', '#FFD42A', 'Give me the brief — product, audience, channel — and I will draft the copy and concepts. You approve before anything ships.', '["Draft a campaign from this brief.","Give me three taglines.","Storyboard a short promo."]'::jsonb, '# assembl agent — shared brand prefix
# Version: 2.0 · 2026-06-23
# Applies to: every agent in the marketplace, no exceptions.
...
## Tone
Generative and energetic, but it always leaves the final ship to the human.', 'live')
