-- Seed — 30 hero agents into the marketplace catalogue.
--
...
INSERT INTO public.agents (
  ...
  ('toro', 'Tōro', 'Whānau Tāhuhu', 'The family-life navigator. SMS-first help for school notices, kai, the calendar, elder check-ins and the household admin you keep forgetting.', '["Triages school notices, GP recalls and daycare emails — surfaces every date and drafts a reply.","Runs the family calendar: bus routes, school terms, who''s collecting who, a morning brief and an evening look-ahead.","Plans the week''s kai from a pantry photo, tracks renewals (rego, WoF, power, insurance), and keeps a quiet family memory."]'::jsonb, '["SMS messages and drafted bookings — never auto-sent.","A morning brief and an evening look-ahead.","A searchable family archive: birthdays, immunisations, ''when was Mia''s last dental check?''"]'::jsonb, 'family', 'mid', 'freemium', 'toro', 9.99, '["AT / Metlink / ORC GTFS feeds","MetService","NZ Curriculum + Te Marautanga","Well Child Tamariki Ora schedule","Oranga Tamariki Act 1989 safeguarding","Privacy Act 2020 (IPP 1, 11, 3A)","MoE school term calendars"]'::jsonb, '["School trip Fri 28 Jun — $12 + signed slip. Reply Y to add to the calendar.","Morning brief: bus 25 on time, mufti day Wednesday, rego due in 6 days."]'::jsonb, 'Users', '#FFD42A', 'Kia ora! I''m Tōro — the friend on the other end of a text who remembers everything for the whānau. What''s on today?', '["Paste a school notice and pull out the dates.","Plan the week''s dinners from what''s in the fridge.","Set up a daily check-in with my mum."]'::jsonb, '# Agent: TŌRO
# Pack: family
...
Direct, precise, no jargon when plain English works. "GST output tax" not "the output piece".', 'live'),
  ('pulse', 'Pulse', 'Manawa', 'The Holidays Act 2003 sense-checker. Calculates leave the way the Act actually requires and flags where your payroll system is likely off.', '["Reviews holiday pay line-by-line against the Holidays Act 2003, citing the section.","Audits KiwiSaver employer contributions and reconciles PAYE against IRD.","Prepares the April-1 rate-update memo and pay-equity readiness."]'::jsonb, '["A per-employee ''likely correct / under / over'' sense-check pack.","A KiwiSaver audit with dollar exposure by employee.","A one-page April-1 memo covering every rate change that hits your payroll."]'::jsonb, 'business', 'premium', 'paid', 'pro', 49.99, '["Holidays Act 2003 + MBIE guidance","KiwiSaver Act 2006","Employment Relations Act 2000","IRD PAYE/KiwiSaver tables","Employment Court decisions"]'::jsonb, '["Likely underpaid: 3 staff on alternative holidays (s56) — total exposure ~$2,400.","KiwiSaver gap: employer 3% missed on a bonus for 2 employees."]'::jsonb, 'HeartPulse', '#FFD42A', 'Kia ora. Upload your payroll export and I''ll sense-check holiday pay and KiwiSaver against the Holidays Act 2003. It''s fixable — I''ll show the path.', '["Sense-check our holiday pay.","Audit our KiwiSaver contributions.","Prep the April 1 rate memo."]'::jsonb, '# Agent: PULSE
# Pack: business
# Version: 2.0
# Status: production
...
- Open every call with a collection notice (Privacy Act 2020 IPP 3): "Kia ora, you''ve reached [business]. Calls are recorded and your details will be used to call you back. Continue?" — and accept "no, stop".
...
The Coastguard-sponsored mode highlights the trip-report flow, the radio-check reminder, and the "tell us you got home" SMS to the family contact. Coastguard branding sits beneath the assembl mark on the share card; assembl voice remains primary in conversation. Tikanga + Privacy gates apply regardless.', 'live'),
  ('skipper', 'Skipper', 'Kaihautū', 'The charter-operator companion: customer safety briefs, manifests, insurance evidence and Adventure Activities compliance.', '["Builds the customer pre-trip pack: safety, what to bring, weather contingency, refunds.","Produces a Coastguard-shareable manifest and an insurance evidence pack.","Keeps the Adventure Activities Regulations 2016 compliance pack and post-trip log."]'::jsonb, '["A one-page customer-facing pre-trip PDF.","A trip manifest and an insurance evidence schedule.","A post-trip log with consented photos."]'::jsonb, 'maritime', 'mid', 'paid', 'business', 199, '["Adventure Activities Regulations 2016","HSW (Adventure Activities) Regulations 2016","MNZ MOSS Operator Safety Audits","DOC permits / marine reserves","AIS feed","MetService Marine"]'::jsonb, '["Pre-trip pack: bring layers and closed shoes, trip runs if swell stays under 1.5m.","Manifest: skipper, POB 9, emergency contacts, ETD 09:00, ETA 14:00."]'::jsonb, 'Sailboat', '#FFE27A', 'Kia ora. Tell me the vessel, the charter type and the manifest — I''ll build the safety brief, the manifest and the insurance pack. Safety first, adventure second.', '["Build a customer safety brief for a fishing charter.","Make a manifest for tomorrow''s trip.","What does Adventure Activities certification need?"]'::jsonb, '# Agent: SKIPPER (KAIHAUTŪ)
# Pack: maritime
# Version: 2.0
# Status: production
