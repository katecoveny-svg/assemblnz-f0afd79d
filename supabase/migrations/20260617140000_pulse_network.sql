-- assembl Pulse — Phase 0 schema (the NZ in-product ad network).
--
-- Three tables back the ad network we dogfood on assembl's own HAPAI tools
-- before pitching any external publisher:
--   pulse_publishers   — who can serve ads (HAPAI is the first publisher)
--   pulse_campaigns    — advertiser demand: bids, budgets, targeting, status
--   pulse_impressions  — one row per served ad, the auditable evidence record
--
-- TRUST CONTRACT (the whole pitch — see project_assembl_pulse_network_locked):
...
--   • We NEVER store a raw IP. pulse_impressions.ip_hash is a salted one-way
--     hash (salt = env PULSE_IP_SALT), used for fraud detection / hourly caps
--     only and irreversible.
--
-- RLS posture:
--   • anon/authenticated may READ pulse_publishers (the SDK needs rev-share /
--     blocklist config and the landing page lists live publishers). They may
--     NOT read pulse_campaigns or pulse_impressions at all.
...
-- ---------------------------------------------------------------------------
-- pulse_publishers — supply side. id is the publisherId the SDK passes us.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.pulse_publishers (
...
-- ---------------------------------------------------------------------------
-- pulse_campaigns — demand side. Advertiser bids, budgets and targeting.
...
CREATE TABLE IF NOT EXISTS public.pulse_campaigns (
  ...
  CONSTRAINT pulse_campaigns_status_chk
    CHECK (status IN ('draft', 'active', 'paused', 'archived')),
  CONSTRAINT pulse_campaigns_bid_chk CHECK (bid_cpm_nzd_cents >= 0),
  CONSTRAINT pulse_campaigns_budget_chk CHECK (daily_budget_nzd_cents >= 0)
);

CREATE INDEX IF NOT EXISTS pulse_campaigns_status_idx ON public.pulse_campaigns (status);

-- ---------------------------------------------------------------------------
-- pulse_impressions — the auditable evidence record. One row per serve attempt:
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.pulse_impressions (
  id                uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  campaign_id       uuid REFERENCES public.pulse_campaigns(id) ON DELETE SET NULL,
  publisher_id      text REFERENCES public.pulse_publishers(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS pulse_impressions_publisher_served_idx
  ON public.pulse_impressions (publisher_id, served_at DESC);
CREATE INDEX IF NOT EXISTS pulse_impressions_campaign_idx
  ON public.pulse_impressions (campaign_id);
-- Backs the per-publisher hourly cap (count rows since now() - 1 hour).
CREATE INDEX IF NOT EXISTS pulse_impressions_served_at_idx
  ON public.pulse_impressions (served_at DESC);

-- ---------------------------------------------------------------------------
-- RLS
-- ---------------------------------------------------------------------------
ALTER TABLE public.pulse_publishers  ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.pulse_campaigns   ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.pulse_impressions ENABLE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS pulse_publishers_read ON public.pulse_publishers;
CREATE POLICY pulse_publishers_read ON public.pulse_publishers
  FOR SELECT TO anon, authenticated USING (active);

-- pulse_campaigns and pulse_impressions get NO anon/authenticated policies on
...
-- ---------------------------------------------------------------------------
INSERT INTO public.pulse_publishers (id, display_name, rev_share_bps, active)
...
--     FROM public.pulse_impressions ORDER BY served_at DESC LIMIT 10;
