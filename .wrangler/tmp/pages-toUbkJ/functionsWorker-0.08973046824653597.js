...
async function onRequestGet(context) {
  ...
  return Response.json({
    ...
    key_length: k.length,
    // a real key is ~108; anything else is a paste problem
...
async function geminiThenWorkers(env, system, messages) {
  if (env.GEMINI_API_KEY) {
    try {
      ...
      lastFailure = (lastFailure ? lastFailure + " \u2192 " : "") + "gemini:" + (r.status + (j && j.error && j.error.status ? ":" + j.error.status : ""));
    } catch (e) {
  ...
  if (env.AI) {
    try {
      ...
      console.error("[agent] workers ai failed:", e);
    }
  }
  return "";
}
__name(geminiThenWorkers, "geminiThenWorkers");
async function onRequestPost(context) {
  ...
  const trimmed = (Array.isArray(messages) ? messages : []).slice(-12).filter((m) => m && (m.role === "user" || m.role === "assistant") && typeof m.content === "string");
  ...
  try {
    if (env.ANTHROPIC_API_KEY) {
      ...
      if (j && Array.isArray(j.content)) {
        ...
        if (answer) {
          lastFailure = null;
          return Response.json({ content: [{ type: "text", text: answer }] }, { headers: { "cache-control": "no-store" } });
        }
      }
  ...
  input_schema: {
    ...
    properties: {
      silhouette: {
        ...
        properties: {
          body: { type: "string", description: "Plain body style \u2014 'two-door coup\xE9', 'estate', 'SUV', 'roadster'." },
          reads_as: { type: "string", description: "What the shape suggests, hedged \u2014 'reads as a classic 356-era coup\xE9', 'reads as a modern performance estate'. Never a flat claim." },
...
var SYSTEM = `You read one vehicle image for a prestige dealership group's
...
var json = /* @__PURE__ */ __name((o, status = 200) => new Response(JSON.stringify(o), {
  status,
  headers: { "content-type": "application/json", "cache-control": "no-store" }
}), "json");
...
// ../../../.wrangler/tmp/pages-toUbkJ/functionsRoutes-0.28629632435891217.mjs
var routes = [
  {
    routePath: "/api/agent",
    mountPath: "/api",
