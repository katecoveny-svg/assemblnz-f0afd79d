...
Before doing substantial work, orient yourself from the repository rather than from remembered chat history.
...
Follow the precedence defined there. Do not treat old chats, old prompts, `CLAUDE.md`, `.Jules/`, legacy code, research outputs, historical handovers, or nearby implementation as higher authority than current repo canon.
...
- do not merge, deploy, publish, send externally, spend money, or perform irreversible/high-consequence actions unless explicitly authorised.
...
## A. Coding / Codex / Claude Code / Jules / software agent
...
Before actions, determine authority: observe / draft / recommend / act-with-approval / act-within-limits. Preview meaningful external or irreversible actions before execution unless explicit authority already exists. Leave a clear receipt of what happened, what changed, evidence, and what remains.
...
Work in `katecoveny-svg/assemblnz-f0afd79d`. Read `START_HERE.md`, `AGENTS.md`, `config/context-manifest.json`, `docs/context/CURRENT.md`, and use `docs/context/README.md` to load only the smallest task-specific canon required. Repo canon beats chat memory, harness memory, research and legacy implementation. Reuse existing primitives. Respect current plum/Instrument Sans brand. State loaded context + proof plan, then execute the task safely without merging/deploying/external actions unless authorised.
