  ...
  function mount(host, opt) {
    ...
    function build() {
      ...
      for (var y = 0; y < rows; y++) {
        for (var x = 0; x < COLS; x++) {
          var v = r2();
          if (v < .70) continue;                     // negative space IS the pattern:
                                                     // roughly seven tiles in ten stay
                                                     // empty. Kate, 1 Aug 2026 — the
                                                     // first build read as a patchwork
                                                     // quilt because it filled.
          var metal = r2();
          tiles.push({
            x: x * S, y: y * S,
            kind: Math.floor(r2() * 6),
            rot: Math.floor(r2() * 4),
            /* one voice, two accents: the brand colour carries the field, ink
               punctuates, brass appears about twice in a hundred tiles. */
            col: metal > .97 ? BRASS : (metal > .88 ? INK : COL),
            soft: r2() > .62,                        // half the field sits back at 45%
    ...
    function paint() {
      ...
      for (var i = 0; i < tiles.length; i++) {
        ...
        var s = S * (.35 + e * .65);
        var off = (S - s) / 2;
        c.globalAlpha = Math.min(1, e * 1.15) * (t.soft ? .42 : 1);
        var isFlip = flip === i;
