...
## 2026-08-04 - Intelligent Character Counter & Polymorphic Form Polish
**Learning:** For polymorphic lead forms (like waitlist vs. talk-to-expert consult forms), adding a real-time character limit and count to open textareas (e.g. use cases) is crucial for expectation setting, but screen reader verbosity must be kept minimal by silencing live region announcements until a critical 90% threshold is met. Additionally, standardizing required indicators with brand-specific asterisks, programmatically setting `aria-required`, and shifting input focus indicators to `focus-visible:` ensures keyboard and screen-reader accessibility without adding visual clutter for mouse interactions.
**Action:** Always implement a character counter with a polite live region that only fires when close to the limit (e.g. 90% threshold). Standardize required labels with explicit brand styling and leverage `focus-visible:` to prevent focus rings on mouse-clicks.
