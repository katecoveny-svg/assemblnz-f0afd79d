    ...
    <main className="min-h-screen bg-[color:var(--assembl-paper)] px-6 py-12 text-[#313c42] md:px-12 md:py-16">
      <div className="mx-auto max-w-[920px]">
        ...
        <section className="mt-8 rounded-[14px] border border-[rgba(35,33,31,0.08)] bg-white p-7">
          <h2 className="border-b border-[#313c42]/20 pb-2 font-display text-2xl font-normal text-[#313c42]">About your organisation</h2>
          <div className="mt-6 grid gap-5">
            ...
            <Field label="What personal information do you collect?">
              <div className="grid gap-2 sm:grid-cols-2">
                {DATA_TYPES.map((item) => (
                  <label key={item} className="flex gap-2 rounded-[8px] p-2 text-sm hover:bg-[#f7f9f8]">
          ...
          <button type="button" onClick={generate} disabled={loading} className="mt-6 rounded-full bg-[#313c42] px-6 py-3 text-sm font-medium text-white hover:bg-[#313c42] disabled:bg-[#C8C2BC]">{loading ? "Generating..." : "Generate one-pager"}</button>
          <div className="mt-3">{gate.counter}</div>
          {gate.modal}
          {loading && <p className="mt-4 rounded-[10px] border border-[#b8964f]/30 bg-[#f7f9f8] px-4 py-3 text-sm text-[#6B5A28]">Mapping your data flows to the Privacy Act 2020.</p>}
        ...
        {html && (
          <>
            <section className="mt-8 rounded-[14px] border border-[rgba(35,33,31,0.08)] bg-white p-8 print:p-0">
              ...
              <div className="mt-6 border-l-4 border-[#b8964f] bg-[#f7f9f8] p-4 text-xs leading-relaxed text-[#5A5550]">
              ...
              <div className="mt-6 flex flex-wrap gap-3 print:hidden">
                <button type="button" onClick={() => window.print()} className="inline-flex items-center gap-2 rounded-full bg-[#313c42] px-5 py-3 text-sm font-medium text-white hover:bg-[#313c42]"><Printer className="h-4 w-4" /> Print / save PDF</button>
                <button type="button" onClick={copyMarkdown} className="inline-flex items-center gap-2 rounded-full border border-[rgba(35,33,31,0.18)] px-5 py-3 text-sm text-[#5A5550] hover:text-[#313c42]"><Copy className="h-4 w-4" /> Copy markdown</button>
              </div>
            </section>

            <section className="mt-8 rounded-[14px] border border-[#313c42]/20 bg-[#f7f9f8] p-7">
              <h2 className="font-display text-2xl font-normal text-[#313c42]">Share this tool</h2>
      ...
      <style jsx global>{`
        .field-input { width: 100%; border-radius: 8px; border: 1px solid rgba(35,33,31,0.12); background: #f7f9f8; padding: 10px 14px; outline: none; }
        .field-input:focus { border-color: #313c42; background: white; }
        .privacy-output h1 { font-family: var(--font-display), Georgia, serif; font-size: 28px; font-weight: 400; color: #313c42; }
        .privacy-output h2 { margin-top: 24px; border-bottom: 1px solid rgba(58,56,50,0.18); padding-bottom: 6px; font-family: var(--font-display), Georgia, serif; font-size: 24px; font-weight: 400; color: #313c42; }
