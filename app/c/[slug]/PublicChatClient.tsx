...
export function PublicChatClient({ tenant, embed = false }: Props) {
  ...
  // assembl Pulse: fill the "Drafting…" wait state with one quiet sponsored
  ...
  const send = useCallback(async () => {
    ...
    // Pulse: ask for an ad to fill this wait state. Coarse context only — the
    ...
    void requestPulseAd({ tool: tenant.slug, kete: tenant.kete, agent: agentSlug || tenant.slug });
  ...
  }, [agentSlug, attachment, chatId, draft, history, redactPii, sending, sessionId, tenant.kete, tenant.slug, requestPulseAd, clearPulseAd]);
