    ...
    <main>
      ...
      <header className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-5 py-5 sm:px-6">
        ...
        <Link
          ...
          style={{ background: 'linear-gradient(135deg, #3A3832, #2A2823)', boxShadow: '0 12px 28px -10px rgba(58,56,50,0.5)', ...display }}
        >
      ...
      <section className="mx-auto grid max-w-6xl items-center gap-10 px-6 pb-8 pt-6 md:grid-cols-[1.05fr_0.95fr] md:pt-12">
        <div>
          <Reveal>
            <span className="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[12px] font-semibold uppercase tracking-[0.14em]" style={{ background: 'var(--b-ochre-soft)', color: 'var(--b-gold-deep)', border: '1px solid rgba(191,163,122,0.28)' }}>
          ...
          <Reveal delay={0.05}>
            <h1 className="mt-5 font-semibold leading-[1.03] tracking-tight" style={{ ...display, color: 'var(--b-ink)', letterSpacing: '0.01em', fontSize: 'clamp(2.35rem, 7.2vw, 3.9rem)' }}>
              ...
              <span style={{ background: 'linear-gradient(90deg, #3A3832, #BFA37A)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', filter: 'none' }}>
                household bills.
              </span>
            </h1>
          </Reveal>
          ...
          <Reveal delay={0.15}>
            <div className="mt-7 flex flex-wrap items-center gap-3">
              <a href="#waitlist" className="inline-flex items-center gap-2 rounded-xl px-6 py-3.5 text-sm font-semibold text-white transition hover:opacity-90" style={{ background: 'linear-gradient(135deg, #3A3832, #2A2823)', boxShadow: '0 14px 32px -10px rgba(58,56,50,0.5)', ...display }}>
          ...
          {flow.map((f, i) => (
            <Reveal key={f.title} delay={(i % 3) * 0.08}>
              <div className="group h-full rounded-2xl p-6 transition duration-300 hover:-translate-y-1" style={{ background: '#FFFFFF', border: '1px solid var(--b-line)', backdropFilter: 'blur(8px)' }}>
                <span className="mb-4 inline-flex h-11 w-11 items-center justify-center rounded-xl transition group-hover:scale-110" style={{ background: 'var(--b-ochre-soft)', color: 'var(--b-gold-deep)', border: '1px solid rgba(191,163,122,0.28)' }}>
                  <f.icon size={20} />
                </span>
