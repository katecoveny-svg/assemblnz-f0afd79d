  ...
  if (!publishableKey) {
    return (
      <div className="rounded-[2px] border border-dashed border-[color:var(--assembl-gold-thread)] bg-white p-4">
        <p className="font-mono text-[11px] uppercase tracking-[0.16em] text-[color:var(--assembl-gold-thread)]">
  ...
  if (!clientSecret) {
    return (
      <div>
        ...
        {error ? (
          <p className="mt-3 font-mono text-[11px] text-[color:#b3261e]">· {error}</p>
        ) : null}
      </div>
    );
  }
    ...
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button
          ...
          onClick={onReset}
          disabled={submitting}
          className="inline-flex h-11 items-center rounded-[2px] border border-[color:var(--assembl-cloud)] px-6 font-mono text-[11px] uppercase tracking-[0.18em] text-[color:var(--text-secondary)] hover:text-[color:var(--text-primary)] disabled:opacity-50"
        >
          cancel
        </button>
      </div>
      {error ? (
        <p className="font-mono text-[11px] text-[color:#b3261e]">· {error}</p>
      ) : null}
    </form>
  );
}
