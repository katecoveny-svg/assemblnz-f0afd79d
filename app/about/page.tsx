  ...
  return <div className={`${styles.page} ${company.page}`}>
    ...
    <section className={styles.hero}>
      ...
      <aside className={styles.heroAside} aria-label="Three connected products">
        {[['Pursuit', 'Find evidence-backed opportunities and prepare the next step.', '/pursuit'], ['DO', 'Give a specialist bounded work, with context and permission attached.', '/do'], ['Studio', 'Turn the possibility into a demonstrator, website, pitch, film or experience.', '/creative-studio']].map(([name, text, href], i) => <div className={styles.heroFact} key={name}><span>0{i + 1}</span><div><strong><Link href={href}>{name} <ArrowUpRight size={16} aria-hidden="true" /></Link></strong><p>{text}</p></div></div>)}
      </aside>
    </section>
