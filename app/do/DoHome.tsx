...
const VIEWS = [
  { name: 'write', eyebrow: 'Your writing and creative agents. Ready when you are.', title: <>Write it.<br />Work it out.</>, copy: 'Draft a reply. Make a plan. Generate an image. Drop in your ingredients and build the agent that helps.' },
  { name: 'work', eyebrow: 'Less organising. A clearer next step.', title: <>A brief.<br />A plan. A reply.</>, copy: 'Compare the options, find the details or prepare the handoff. Build a writing or creative agent around the material you choose.' },
...
export function DoHome() {
  ...
  return <div className="do-product">
    <section className={`do-world ${cinema ? 'do-cinema' : ''}`} aria-label="DO, by assembl">
      ...
      <div className="do-world-story do-interface" key={view}><p>{VIEWS[view].eyebrow}</p><h1>{VIEWS[view].title}</h1><p className="do-world-lede">{VIEWS[view].copy}</p><button className="do-world-make" onClick={openWorkspace}><span aria-hidden>✦</span> Build an agent <ArrowUpRight size={20} /></button><span className="do-world-boundary">Prepare · review · take it with you</span></div>
      <aside className="do-world-peek do-interface"><span className="do-small-label">BUILD YOUR OWN AGENT</span><button onClick={openWorkspace}><span aria-hidden>✦</span><span><strong>{view === 0 ? 'A reply. A plan. A clearer draft.' : view === 1 ? 'A clearer comparison.' : 'A brief ready for your review.'}</strong><small>Pick an agent. Add text. Take the result with you.</small></span><ArrowUpRight size={19} /></button></aside>
