  ...
  return <div className={styles.shell}>
    ...
    <main className={styles.main}>
      ...
      {GROUPS.map((group) => {
        ...
        return <section className={styles.group} key={group}><header><span>{group}</span><p>{group === 'communication' ? 'mail and messaging context' : group === 'work' ? 'systems where work lands' : group === 'creative' ? 'production capabilities included by the platform' : 'spatial and interactive production'}</p></header><div className={styles.grid}>{cards.map((capability) => <article className={styles.card} key={capability.key}><div className={styles.cardTop}><div className={styles.icon}><PlugZap size={17}/></div><span data-status={capability.status}>{capability.status}</span></div><h2>{capability.label}</h2><p>{capability.description}</p><small>{capability.authority.replace('_', ' ')}</small>{capability.apps?.length ? <div className={styles.apps}>{capability.apps.map((app) => { const isConnected = connected.has(app.slug); return <button type="button" key={app.slug} disabled={!state?.signedIn || !state?.availability[app.slug] || state.accountsAvailable === false || isConnected || Boolean(busy)} onClick={() => void connect(app.slug)}>{isConnected ? <><CheckCircle2 size={14}/> {app.label} connected</> : busy === app.slug ? `opening ${app.label}…` : !state?.availability[app.slug] ? `${app.label} · setup needed` : `Connect ${app.label}`}</button>; })}</div> : <div className={styles.included}>{capability.status === 'preview' ? 'Preview · not yet connected to DO' : 'Platform capability · availability depends on the task'}</div>}</article>)}</div></section>;
      })}
