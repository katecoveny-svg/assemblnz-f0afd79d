...
describe('DO downloadable product', () => {
  it('packages a working extension with D-mark icons, side panel and valid JS', async () => {
    ...
    expect(response.headers.get('Content-Disposition')).toContain('assembl-do-extension-1.5.0.zip');
...
describe('extension JS package hygiene', () => {
  it('every top-level extension .js quotes use strict and parses', () => {
    const files = readdirSync(EXTENSION_DIR).filter((name) => name.endsWith('.js'));
