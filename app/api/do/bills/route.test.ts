...
describe('bill research service',()=>{
 ...
 it('refunds an ungrounded provider answer',async()=>{fetcher.mockResolvedValue(Response.json({candidates:[{finishReason:'STOP',content:{parts:[{text:'An unsupported cheap plan'}]}}]}));expect((await POST(req())).status).toBe(503);expect(mocks.release).toHaveBeenCalledOnce();});
 it('uses the live-search tool and returns the provider source evidence',async()=>{fetcher.mockResolvedValue(Response.json({candidates:[{finishReason:'STOP',content:{parts:[{text:'Review a current advertised offer'}]},groundingMetadata:{webSearchQueries:['NZ broadband offers'],groundingChunks:[{web:{uri:'https://provider.example/offer',title:'Provider'}}]}}]}));const response=await POST(req());expect(response.status).toBe(200);expect((await response.json()).sources).toHaveLength(1);expect(JSON.parse(fetcher.mock.calls[0][1].body).tools).toEqual([{google_search:{}}]);expect(mocks.release).not.toHaveBeenCalled();});
});
