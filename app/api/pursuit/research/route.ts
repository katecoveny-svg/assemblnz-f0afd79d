...
export async function POST(request:Request){
  ...
  try{
    ...
    const hash=createHash('sha256').update(JSON.stringify(input.data)).digest('hex');
    ...
    if(reservation.status!=='reserved'){
      ...
      return Response.json({error:limited?'The free research limit has been reached. Try tomorrow or contact assembl.':reservation.status==='disabled'?'Live research is not enabled yet.':'This request is already being handled. Start a new brief to try again.',code:reservation.status},{status:limited?429:reservation.status==='disabled'?503:409,headers:noStore});
    }
    id=input.data.requestId;
    const result=await runPublicResearch(input.data,Boolean(reservation.typesafeEnabled));
    await completeTrial(id,principal,result);
    await countPublicTool('public_pursuit_complete');
