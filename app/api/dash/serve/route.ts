/**
 * POST /api/dash/serve — the Dash by assembl ad server.
 *
 * Runs a second-price auction (lib/dash/auction) over active campaigns for the
 ...
 * stored — only a salted one-way hash (lib/dash/ip).
 */
...
import {
  ...
  spentToday,
  type DashCampaign,
} from '@/lib/dash/auction';
import { clientIp, hashIp } from '@/lib/dash/ip';

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

/** Per-publisher hourly impression cap. Override via env; defaults to 1000. */
const HOURLY_CAP = Number(process.env.DASH_HOURLY_CAP ?? 1000);
...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error('[dash/serve] service client unavailable:', err);
  ...
  const result = runAuction((campaigns ?? []) as DashCampaign[], {
    publisherId,
    surface,
  ...
  if (impErr || !impression) {
    console.error('[dash/serve] impression insert failed:', impErr?.message);
    return NO_FILL;
  }
