/**
 * POST /api/dash/lead — "I'm a publisher" / "I want to advertise" capture for
 * the /dash (Dash by assembl) landing page.
 *
 ...
 * lead_inquiries is the site's canonical, fail-soft lead path, so every Dash
...
export async function POST(req: Request) {
  ...
  if (!result.notified && !result.persisted) {
    console.error('[dash/lead] both legs failed', { role, email });
    return NextResponse.json(
