...
const generateContract = (type: string, company: string) => {
  ...
  const disclaimer = `\n\n---\n DISCLAIMER: This template is for guidance only. Have it reviewed by an employment lawyer before use. AROHA provides general information, not legal advice.\n\nThe employee has the right to seek independent advice before signing this agreement (s 63A Employment Relations Act 2000).`;
...
const generateOtherDoc = (docType: string, company: string) => {
  ...
  const header = `${company || "[Company Name]"}\n${date}\nRef: ${ref}\n\n`;
  const disclaimer = `\n\n---\n This template is for guidance only. Have it reviewed by an employment lawyer before use.`;
