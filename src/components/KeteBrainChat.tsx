                ...
                <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3" style={{ minHeight: 120 }}>
                  {messages.length === 0 && (
                    <div className="text-center py-6">
                      <div className="flex justify-center"><BrainAvatar color={accentColor} size={56} /></div>
                      <p className="text-xs mt-3" style={{ color: "#3D4250" }}>Kia ora! I'm the {keteName} specialist.</p>
                      <p className="text-[10px] mt-1" style={{ color: "#9CA3AF" }}>Tap a starter or ask me anything about {keteNameEn.toLowerCase()}.</p>
                      <div className="mt-4 flex flex-col gap-2 px-1">
                        {getStarterPrompts(keteId, agentId).map((p) => (
                          <button
                            key={p}
                            type="button"
                            onClick={() => sendMessage(p)}
                            disabled={isStreaming}
                            className="text-left text-xs px-3 py-2 rounded-xl transition-all hover:-translate-y-px disabled:opacity-50"
                            style={{
                              background: "#FFFFFF",
                              border: `1px solid ${hexRgba(accentColor, 0.25)}`,
                              color: "#3D4250",
