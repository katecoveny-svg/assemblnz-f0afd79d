...
import { Ship, FileCheck, Calculator, Bug, AlertTriangle, Globe, Menu, X, ChevronLeft, ChevronRight } from "lucide-react";

const ACCENT = "#B8C7B1"; // Soft Moss

const NAV = [
  { label: "Shipments", to: "/pikau/dashboard", icon: Ship, end: true },
  { label: "Customs", to: "/pikau/dashboard/customs", icon: FileCheck },
  { label: "Landed cost", to: "/pikau/dashboard/landed-cost", icon: Calculator },
  { label: "Biosecurity", to: "/pikau/dashboard/biosecurity", icon: Bug },
  { label: "Dangerous goods", to: "/pikau/dashboard/dangerous-goods", icon: AlertTriangle },
  { label: "FTA checker", to: "/pikau/dashboard/fta", icon: Globe },
];

export default function PikauLayout() {
    ...
    <div className="flex flex-col h-full">
      <div className="p-5 border-b border-[#D8C8B4]/30">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: `linear-gradient(135deg, ${ACCENT}55, ${ACCENT}22)` }}>
            <Ship size={20} className="text-[#9D8C7D]" />
          </div>
          {!collapsed && (
            <div>
              <h2 className="font-display text-lg text-[#6F6158] tracking-wide">PIKAU</h2>
              <p className="font-body text-[10px] text-[#9D8C7D]">Freight & customs</p>
            </div>
          )}
        </div>
      </div>
