  ...
  if (!user) return (
    <div className="min-h-screen bg-[#09090F] flex items-center justify-center text-center space-y-4">
      <div>
        ...
        <p className="text-white/70 font-jakarta mb-4">Sign in to access your Landlord Portal</p>
      ...
      <header className="border-b border-white/5 px-4 py-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-syne font-bold text-lg">Landlord Portal</h1>
              <p className="text-xs font-jakarta" style={{ color: "#FF80AB" }}>HAVEN by Assembl</p>
            </div>
          </div>
        </div>
      </header>
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                <div key={stat.label} className="rounded-xl p-4 text-center" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
                  <div className="text-2xl font-syne font-bold" style={{ color: stat.color }}>{stat.value}</div>
            ...
            {openJobs.length > 0 && (
              <div className="space-y-2">
                <h3 className="font-syne font-bold text-sm">Open Jobs</h3>
            ...
            {overdueCompliance.length > 0 && (
              <div className="space-y-2">
                <h3 className="font-syne font-bold text-sm flex items-center gap-1.5"><AlertTriangle size={14} color="#EF5350" /> Overdue Compliance</h3>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {properties.length === 0 ? (
              ...
              <div key={p.id} className="rounded-xl p-4 space-y-2" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
                <h3 className="font-syne font-bold text-sm">{p.address}</h3>
            ...
            {jobs.length === 0 ? (
              ...
              <div key={job.id} className="rounded-xl p-4 space-y-2" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
                <div className="flex items-start justify-between">
                  <h3 className="font-syne font-bold text-sm">{job.title}</h3>
        ...
        {activeTab === "costs" && (
          <div className="space-y-4">
            <div className="rounded-xl p-6 text-center" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
              <div className="text-3xl font-syne font-bold" style={{ color: "#66BB6A" }}>
