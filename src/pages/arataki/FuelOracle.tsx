...
import { Fuel, MapPin, TrendingDown, ArrowRight } from "lucide-react";
    ...
    <div style={{ background: "#09090F", minHeight: "100vh", color: "#1A1D29" }}>
      ...
      <main className="max-w-5xl mx-auto px-6 pt-16 pb-32">
        ...
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-3xl md:text-4xl font-light mb-3" style={{ fontFamily: "'Lato', sans-serif" }}>
          <Fuel className="inline mr-3 text-emerald-400" size={28} />FuelOracle
        </motion.h1>
        <p className="text-white/50 mb-10 max-w-xl">Live NZ fuel pricing across Z, BP, Mobil, Gull, Waitomo — automatically selects cheapest stops on your route.</p>

        <div className="rounded-2xl overflow-hidden" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
          <div className="grid grid-cols-5 text-xs font-semibold text-white/40 uppercase tracking-wider px-6 py-3 border-b border-white/5">
          ...
          {FUEL_DATA.map((f, i) => (
            <motion.div key={f.station} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.1 }}
              className={`grid grid-cols-5 px-6 py-4 text-sm ${f === cheapest ? "bg-emerald-500/5" : ""} border-b border-white/[0.03]`}>
              <span className="text-white/80 flex items-center gap-2"><MapPin size={12} className="text-white/20" />{f.station}</span>
              <span className="text-white/50">{f.brand}</span>
              <span className={f === cheapest ? "text-emerald-400 font-semibold" : "text-white/70"}>${f.diesel.toFixed(3)}</span>
              <span className="text-white/50">${f.petrol91.toFixed(3)}</span>
              <span className="text-white/30">{f.updated}</span>
            </motion.div>
          ))}
        </div>
        <div className="mt-6 p-4 rounded-xl flex items-center gap-3" style={{ background: "rgba(16,185,129,0.06)", border: "1px solid rgba(16,185,129,0.15)" }}>
          <TrendingDown size={16} className="text-emerald-400" />
          <span className="text-sm text-emerald-300">Best diesel: <strong>{cheapest.station}</strong> at ${cheapest.diesel.toFixed(3)}/L — saves ~$4.20 per fill vs most expensive</span>
        </div>
      </main>
      <BrandFooter />
    </div>
