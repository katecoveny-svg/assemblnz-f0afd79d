/**
 * Salted, one-way IP hashing for Dash by assembl fraud detection.
 *
 ...
 * reversed. The salt lives in env DASH_IP_SALT (server-only).
 *
...
export function hashIp(ip: string | null | undefined): string | null {
  const salt = process.env.DASH_IP_SALT;
  if (!salt) {
    console.warn('[dash] DASH_IP_SALT is not set — impressions will have no ip_hash.');
    return null;
  }
  if (!ip) return null;
