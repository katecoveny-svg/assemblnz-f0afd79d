/**
 * Dash by assembl — the second-price auction core.
 *
 * Pure, dependency-free, and unit-tested in auction.test.ts. The serve route
 * (app/api/dash/serve) does the I/O — load publisher + campaigns, then call
 ...
 */
export function spentToday(c: DashCampaign, nzToday: string): number {
  return c.spent_today_date === nzToday ? c.spent_today : 0;
}

/** Is this campaign eligible to bid for this impression? */
export function isEligible(c: DashCampaign, ctx: AuctionContext): boolean {
 ...
 */
export function runAuction(campaigns: DashCampaign[], ctx: AuctionContext): AuctionResult | null {
