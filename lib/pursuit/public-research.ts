...
export async function runPublicResearch(input:TrialInput,allowTypeSafe:boolean,fetcher:typeof fetch=fetch):Promise<PublicResearchResult>{
 ...
 const sources=new Map<string,EvidenceSource>();const messages:unknown[]=[{role:'user',content:JSON.stringify({company:input.company,goal:input.goal,currentDate:at.slice(0,10),publishedAssemblKnowledge:knowledge,instruction:'Find one useful opening. Search public sources and return the JSON draft.'})}];
