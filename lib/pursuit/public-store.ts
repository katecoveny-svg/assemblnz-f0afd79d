...
async function db(path:string,method='GET',body?:unknown){if(!storageConfigured())throw new Error('storage_unavailable');const response=await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/${path}`,{method,cache:'no-store',redirect:'error',signal:AbortSignal.timeout(5000),headers:{apikey:process.env.SUPABASE_SERVICE_ROLE_KEY!,Authorization:`Bearer ${process.env.SUPABASE_SERVICE_ROLE_KEY!}`,'Content-Type':'application/json',Prefer:'return=representation'},...(body!==undefined?{body:JSON.stringify(body)}:{})});if(!response.ok)throw new Error('storage_unavailable');return response.status===204?null:response.json();}
export async function trialPolicy(){try{const rows=await db('pursuit_public_policy?id=eq.true&select=enabled,typesafe_enabled,global_daily_limit,client_daily_limit');return Array.isArray(rows)?rows[0]??null:null;}catch{return null;}}
export async function trialUsage(principal: string) {
 const start = new Date(); start.setUTCHours(0,0,0,0);
 try {
  // The policy has a hard maximum of 100 site-wide attempts. Only hashes stay server-side.
  const rows = await db(`pursuit_public_runs?created_at=gte.${encodeURIComponent(start.toISOString())}&select=principal_hash&limit=101`);
  if (!Array.isArray(rows)) return null;
  return { global: rows.length, client: rows.filter(row => row.principal_hash === principal).length };
 } catch { return null; }
}
export function nextTrialReset() {
 const reset = new Date(); reset.setUTCHours(24,0,0,0); return reset.toISOString();
}
