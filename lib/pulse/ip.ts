/**
 * Salted, one-way IP hashing for assembl Pulse fraud detection.
 *
 ...
 * reversed. The salt lives in env PULSE_IP_SALT (server-only).
 *
...
export function hashIp(ip: string | null | undefined): string | null {
  const salt = process.env.PULSE_IP_SALT;
  if (!salt) {
    console.warn('[pulse] PULSE_IP_SALT is not set — impressions will have no ip_hash.');
    return null;
  }
  if (!ip) return null;
