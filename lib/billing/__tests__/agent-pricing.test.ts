...
describe('agent pricing ladder', () => {
  ...
  it('reports the agent pick-count for each plan', () => {
    ...
    expect(agentCountForPlan('prostack')).toBe(0); // fixed bundle — no pick
  ...
  it('labels each tier for display', () => {
    ...
    expect(agentPriceLabel('prostack')).toBe('Pro Stack — $49/mo');
  ...
  it('resolves price ids from env and reverses them', () => {
    ...
    expect(planForPriceId('price_prostack', env)).toBe('prostack');
  ...
  it('guards unknown plan ids', () => {
    ...
    expect(isAgentPlan('prostack')).toBe(true);
    ...
    expect(isAgentPlan('whanau')).toBe(false); // dead tier
    expect(isAgentPlan('pro')).toBe(false); // dead tier
    expect(getAgentPlan('enterprise')).toBeUndefined();
  });

  it('defines the July promo and the Pro Stack bundle', () => {
    ...
    expect(PROSTACK_BUNDLE_SLUGS).toHaveLength(4);
  });
});
