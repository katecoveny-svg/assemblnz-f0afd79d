/**
 * Auaha (creative) policy pack — flagship deep-port.
 *
 * Ported verbatim from `assemblnz-latest/src/aaaip/policy/auaha.ts`. Six rules
 * the creative agent's `publish_asset` action is gated by before anything ships:
 * copyright/licence, likeness consent, brand safety, te reo integrity,
 * misinformation, and the uncertainty hand-off. Feed these to a
 * {@link ComplianceEngine} to reproduce the old draft-and-suggest behaviour.
 */
...
const brandSafetyPredicate: PolicyPredicate = (action) => {
  ...
  if (risk > 0.6) {
    return fail(BRAND_SAFETY.id, 'warn', `Brand risk ${risk.toFixed(2)} > 0.60 — request brand-manager review.`);
  }
...
const teReoPredicate: PolicyPredicate = (action) => {
  ...
  if (teReo && kaitiakiReview !== true) {
    return fail(TE_REO_INTEGRITY.id, 'warn', 'Asset contains te reo but has not been reviewed by a kaitiaki.');
  }
...
const misinfoPredicate: PolicyPredicate = (action) => {
  ...
  if (factScore < 0.55) {
    return fail(MISINFO.id, 'warn', `Fact confidence ${factScore.toFixed(2)} < 0.55 — request review.`);
  }
