"""Mechanical repair of inspected legacy DO colours; task and auth logic untouched."""
...
def replace_exact(text,old,new,count=1):
 if text.count(old)!=count:raise RuntimeError('Expected exact source fragment changed: '+old[:80])
 return text.replace(old,new)
p=Path('app/do/DoCanvas.tsx');s=p.read_text()
s=replace_exact(s,'{ plum: "#d5a6e8", violet: "#aaa5ff", copper: "#e3ad91" }','{ plum: "#916A70", violet: "#654A4E", copper: "#F5F1F2" }')
s=replace_exact(s,'src="/do/canvas/dimensional-d.png"','src="/do/canvas/identity-plum.svg"')
s=replace_exact(s,'alt="Dimensional purple D and glowing dot"','alt="DO symbol in deep plum, paper and rose"')
# Appearance panel and floating widget: two verified refs in this source SHA.
s=replace_exact(s,'src="/do/canvas/pet-do-purple.png"','src="/do/canvas/pet-do-plum.svg"',2)
s=replace_exact(s,'                      {c}','                      {{ plum: "rose", violet: "plum", copper: "chalk" }[c]}')
p.write_text(s)
p=Path('app/do/DoAppearance.tsx');s=p.read_text()
s=replace_exact(s,'const colours = { plum: "#916A70", violet: "#654A4E", copper: "#C4A494" };','// Saved colour keys remain compatible; visible accents use the current brand.\n  const colours = { plum: "#916A70", violet: "#654A4E", copper: "#F5F1F2" };')
s=replace_exact(s,'<legend>Colour</legend>','<legend>Accent</legend>')
s=replace_exact(s,'              {c}','              {{ plum: "rose", violet: "plum", copper: "chalk" }[c]}')
p.write_text(s)
mapping={'1e0e23':'240B21','faf4fa':'FFFDFB','ccb9cf':'F5F1F2','674770':'654A4E','f6edf7':'FFFDFB','fff8ff':'FFFDFB','25152b':'240B21','fff7ff':'FFFDFB','d6c2dd':'F5F1F2','9969ad':'916A70','35213f':'3B2035','c79ae2':'D6A5BD','ceb7d5':'F5F1F2','50365c':'654A4E','ab8ab5':'D6A5BD','765083':'916A70','302037':'3B2035','a977be':'916A70','faf6fa':'FFFDFB','321a3b':'240B21','decede':'E5D9DF','7b6085':'654A4E','7a6383':'654A4E','d3bddb':'D6A5BD','ddcfe3':'E5D9DF','43234e':'240B21','42204e':'240B21','42284e':'3B2035','79518b':'916A70','402051':'240B21','ddb4fc':'FFFDFB','b571ff':'916A70','aa62dc':'916A70','d6a0ff':'D6A5BD','24122e':'240B21','9869b0':'916A70','eee1f5':'FFFDFB','c896ea':'D6A5BD','d4bfdb':'F5F1F2','f7f2f7':'F5F1F2','f9efff':'FFFDFB','301539':'240B21','210f24':'240B21','694061':'654A4E','532c4d':'654A4E','b57fba':'916A70','805274':'654A4E','d6a0c4':'D6A5BD'}
p=Path('app/do/do-canvas.css');s=p.read_text();s=re.sub(r'#([0-9a-fA-F]{6})([0-9a-fA-F]{2})?\b',lambda m:'#'+mapping.get(m[1].lower(),m[1])+(m[2] or ''),s);s=s.replace('mix-blend-mode: screen;','mix-blend-mode: normal;');p.write_text(s)
...
print('Repaired assets/accents/CSS. Stored appearance keys, task behaviour and permissions unchanged.')
