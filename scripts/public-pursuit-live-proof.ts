/** One bounded live test. No private brief or credentials. No automatic provider retries. */
...
const root='https://www.assembl.co.nz';const out='visual-evidence/live-pursuit';
...
async function main(){
 ...
 const status=await json('/api/pursuit/research');proof.availability=status;assert.equal(status.ready,true,'Explicit test activation required');assert.ok(status.limits.globalDaily<=3&&status.limits.perClientDaily<=1,'Trial exceeds test caps');
 const knowledge=await json('/api/knowledge/search?q=Pursuit%20Studio');proof.knowledge=knowledge;assert.equal(knowledge.paid,false);assert.equal(knowledge.privateKnowledge,false);assert.ok(knowledge.records.length>0);
 ...
 const tool=await json('/api/knowledge/mcp',{jsonrpc:'2.0',id:2,method:'tools/call',params:{name:'search_assembl_public_knowledge',arguments:{query:'Pursuit Studio'}}});assert.ok(tool.result&&!tool.result.isError);proof.mcpSearch=tool;
 ...
 proof.requestId=input.requestId;await writeFile(out+'/status.json',JSON.stringify(proof,null,2));
 ...
 const urls=new Set(result.trace.sources.map(s=>s.url));for(const evidence of result.draft.evidence)assert.ok(urls.has(evidence.url),'Untraced source');
 const deck=buildPitchHtml(result);assert.equal((deck.match(/<section id="slide-/g)??[]).length,6);assert.ok(!deck.includes('<script'));await writeFile(out+'/bunnings-independent-pitch.html',deck);
 const replay=await json('/api/pursuit/research',input);assert.deepEqual(replay,result,'Replay must not rerun the provider');
 proof.passed=true;proof.completedAt=new Date().toISOString();proof.trace=result.trace;proof.replay='identical persisted response';await writeFile(out+'/status.json',JSON.stringify(proof,null,2));console.log(JSON.stringify({passed:true,requestId:input.requestId,model:result.trace.model,webSearches:result.trace.webSearches,knowledgeIds:result.trace.knowledgeIds,sourceCount:result.trace.sources.length,replay:proof.replay}));
}
