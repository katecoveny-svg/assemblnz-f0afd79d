...
import { IconMenuPOS, IconFoodSafety, IconGuestMemory, IconReservation, IconStaffRoster, IconRevenue } from "../shared/BrandIcons";

const { fontFamily } = loadFont("normal", { weights: ["400", "700", "900"], subsets: ["latin"] });

const FEATURES = [
  { Icon: IconMenuPOS, title: "Menu & POS Intelligence", desc: "Pricing, costing & real-time analytics" },
  { Icon: IconFoodSafety, title: "Food Safety Diary", desc: "NZ Food Act 2014 digital compliance" },
  { Icon: IconGuestMemory, title: "Guest Memory", desc: "Preferences, allergies & VIP profiles" },
  { Icon: IconReservation, title: "Reservations", desc: "Smart booking & table management" },
  { Icon: IconStaffRoster, title: "Staff Rostering", desc: "Scheduling, training & payroll" },
  { Icon: IconRevenue, title: "Revenue Management", desc: "Dynamic pricing & occupancy AI" },
];
...
export const AuraStory: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ background: cosmicGradient, fontFamily, overflow: "hidden" }}>
      <GlassOrb x={120} y={350} size={200} color={AURA_ACCENT} delay={0} />
      <GlassOrb x={750} y={550} size={140} color={BRAND.purple} delay={2} />
      <GlassOrb x={200} y={1400} size={160} color={BRAND.deepBlue} delay={4} />
      <GlassOrb x={850} y={1750} size={100} color={BRAND.cyan} delay={6} />
      ...
      <Sequence from={0}>
        <div style={{
          ...
          <div style={{
            width: 200, height: 200, borderRadius: 40, overflow: "hidden",
            background: `linear-gradient(135deg, ${AURA_ACCENT}15, ${BRAND.purple}10)`,
            border: `2px solid ${AURA_ACCENT}40`,
            boxShadow: `0 0 ${avatarGlow}px ${AURA_ACCENT}50, 0 20px 60px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.15)`,
            transform: `scale(${avatarIn}) translateY(${avatarFloat}px)`,
          }}>
            <Img src={staticFile("images/aura-avatar.png")} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
          </div>
          ...
          {FEATURES.map((f, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                opacity: cardIn,
                boxShadow: `0 8px 32px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.1), 0 0 16px ${AURA_ACCENT}${Math.round(interpolate(Math.sin(frame * 0.03 + i), [-1, 1], [0, 0.2]) * 255).toString(16).padStart(2, "0")}`,
              }}>
                <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
                  <div style={{
                    width: 56, height: 56, borderRadius: 16,
                    background: `linear-gradient(135deg, ${AURA_ACCENT}12, ${BRAND.purple}08)`,
                    display: "flex", alignItems: "center", justifyContent: "center",
                    border: `1px solid ${AURA_ACCENT}25`,
                    flexShrink: 0,
                  }}>
                    <f.Icon size={36} color={AURA_ACCENT} glow={iconGlow} />
                  </div>
                  <div>
            ...
            return (
              <>
                <div style={{
                  ...
                  border: `1px solid ${AURA_ACCENT}40`,
                  transform: `scale(${ctaIn})`, opacity: ctaIn,
                }}>
                  <div style={{ fontSize: 18, fontWeight: 700, color: BRAND.white }}>assembl.co.nz/chat/hospitality</div>
                </div>
                <div style={{
