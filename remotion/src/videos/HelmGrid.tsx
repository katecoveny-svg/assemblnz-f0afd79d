...
import { BRAND, HELM_ACCENT, cosmicGradient, glassCardStyle } from "../shared/styles";
import { IconCalendarSync, IconMealPlan, IconBudgetTrack, IconNewsExtract, IconBusTracker, IconFamilyHub } from "../shared/BrandIcons";

const { fontFamily } = loadFont("normal", { weights: ["400", "700", "900"], subsets: ["latin"] });

const CAPS = [
  { Icon: IconCalendarSync, label: "Calendar Sync" },
  { Icon: IconMealPlan, label: "Meal Planner" },
  { Icon: IconBudgetTrack, label: "Budget Track" },
  { Icon: IconNewsExtract, label: "News Extract" },
  { Icon: IconBusTracker, label: "Bus Tracker" },
  { Icon: IconFamilyHub, label: "Family Hub" },
];

export const HelmGrid: React.FC = () => {
        ...
        {[0, 60, 120, 180, 240, 300].map((deg, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              background: i % 3 === 0 ? HELM_ACCENT : i % 3 === 1 ? BRAND.cyan : BRAND.emerald,
              boxShadow: `0 0 ${dotPulse * 2}px ${i % 3 === 0 ? HELM_ACCENT : i % 3 === 1 ? BRAND.cyan : BRAND.emerald}70`,
            }} />
          );
        })}
        <svg width={700} height={700} style={{ position: "absolute", top: 0, left: 0 }}>
          {[0, 60, 120, 180, 240, 300].map((deg, i) => {
            ...
            return <line key={i}
              ...
              stroke={`${HELM_ACCENT}20`} strokeWidth={1} />;
          })}
        </svg>
      </div>
      ...
      <Sequence from={0}>
        <div style={{
          ...
          <div style={{
            ...
            background: `linear-gradient(135deg, ${HELM_ACCENT}12, ${BRAND.cyan}08)`,
            border: `2px solid ${HELM_ACCENT}35`,
            boxShadow: `0 0 ${avatarGlow}px ${HELM_ACCENT}40, 0 16px 48px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.12)`,
            transform: `scale(${heroScale}) translateY(${avatarFloat}px)`,
            marginBottom: 20,
          }}>
            <Img src={staticFile("images/helm-avatar.png")} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
          </div>

          <div style={{
            fontSize: 80, fontWeight: 900, letterSpacing: -3,
            background: `linear-gradient(135deg, ${HELM_ACCENT}, ${BRAND.cyan}, ${BRAND.emerald})`,
          ...
          {CAPS.map((cap, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                <cap.Icon size={28} color={HELM_ACCENT} glow={iconGlow} />
